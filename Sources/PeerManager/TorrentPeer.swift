...
protocol TorrentPeerDelegate: AnyObject {
...
class TorrentPeer {
    ...
    private var downloadPieceRequests: [Int: TorrentPieceBuffer] = [:]
    private var numberOfPendingBlockRequests = 0
    
    private var uploadPieceRequests: [Int: TorrentPieceRequestBuffer] = [:]
    ...
    func downloadPiece(atIndex index: Int, size: Int) {
        let downloadBuffer = TorrentPieceBuffer(index: index, size: size)
    ...
    private func sendNextBlock() {
        ...
        communicator.sendPiece(fromPieceAtIndex: block.request.piece, begin: block.request.begin, block: block.data) { [weak self] in
            self?.currentlySendingBlock = false
            self?.sendNextBlock()
        }
    }
    
    private func appendBlockRequest(_ blockRequest: TorrentBlock.Request) {
        let index =  blockRequest.piece
        var pieceRequest: TorrentPieceRequestBuffer
        if let previousRequest = uploadPieceRequests[index] {
            ...
            guard let pieceData = delegate?.peer(self, requestedPieceAtIndex: index) else {
                print ("Error: peer asked for a piece I don't have")
                return
            }
            pieceRequest = TorrentPieceRequestBuffer(data: pieceData, index: index)
    ...
    private func cancelBlockRequest(_ blockRequest: TorrentBlock.Request) {
...
extension TorrentPeer {
    ...
    private func waitForKeepAlive() {
        keepAliveTimer = Timer.scheduledTimer(timeInterval: keepAliveTimeout, target: self, selector: #selector(TorrentPeer.didntReceiveKeepAlive), userInfo: nil, repeats: false)
    }
...
extension TorrentPeer: TorrentPeerCommunicatorDelegate {
    func peerConnected(_ sender: TorrentPeerCommunicator) {
        print("Info: Peer socket connected (\(peerInfo.ip):\(peerInfo.port)")
    ...
    func peer(_ sender: TorrentPeerCommunicator, hasPiece piece: Int) {
        guard !currentProgress.checkAvailability(at: piece) else { return }
        currentProgress.setBit(at: piece, with: true)
    ...
    func peer(_ sender: TorrentPeerCommunicator, hasBitFieldData bitFieldData: Data) {
        ...
        do {
            currentProgress = try BitField(data: bitFieldData, size: bitFieldSize)
            delegate?.peerHasNewAvailablePieces(self)
        } catch {
    ...
    func peer(_ sender: TorrentPeerCommunicator, requestedPiece index: Int, begin: Int, length: Int) {
        let blockRequest = TorrentBlock.Request(piece: index, begin: begin, length: length)
    ...
    func peer(_ sender: TorrentPeerCommunicator, sentPiece index: Int, begin: Int, block: Data) {
        ...
        if downloadPieceBuffer.isComplete, let piece = downloadPieceBuffer.piece {
            print("Info: Got entire piece \(index)")
